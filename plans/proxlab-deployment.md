...
- [x] Verify `vmbr0` (management, <mgmt-ip>/23) still up after vmbr1 added
...
- [x] Build Ubuntu 24.04 cloud-init template — **ID 22200** `ubuntu-2404-tmpl` on `vms-1tb`
  - q35, OVMF/UEFI, virtio-scsi, cloud-init drive, DNS → your management network DNS
...
- [x] Create `claude-lxc` (LXC, Ubuntu 24.04, 4 GB RAM, CT 22202)
  ...
  - eth1: vmbr0, `<mgmt-ip>/24`, gw `<mgmt-gw>` (management network / internet)
...
- [x] Verify claude-lxc can reach management network (ping <mgmt-ip> ✅)
...
- [ ] Configure DHCP scope for VLAN 100:
  ...
  ```
- [ ] Set DNS forwarder for external resolution:
  ```powershell
  Set-DnsServerForwarder -IPAddress <your-dns-server> -UseRootHint $false
...
### 10.3 Additional SecOps targets

Deploy selectively as needed from your ISO storage.
